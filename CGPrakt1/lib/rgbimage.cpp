...
void RGBImage::setPixelColor(unsigned int x, unsigned int y, const Color &c) {
    if (x < 0 || x > width() || y < 0 || y > height()) {
        throw std::invalid_argument("falsche Pixelwerte erhalten");
    } else {
        m_Image[y * width() + x] = c;
    }
}
...
bool RGBImage::saveToDisk(const char *Filename) {
    ...
    for (int i = 0; i < width(); i++) {
        for (int j = 0; j < height(); j++) {
            ...
            /* OLD
            unsigned char r = convertColorChannel(m_Image[i + j * this->m_Width].R);
            unsigned char g = convertColorChannel(m_Image[i + j * this->m_Width].G);
            unsigned char b = convertColorChannel(m_Image[i + j * this->m_Width].B);
            */
