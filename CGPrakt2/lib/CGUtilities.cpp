...
public:
    ...
    float value(float u, float v, float w) {
        ...
        for (int a = 0; a < 2; a++)
            for (int b = 0; b < 2; b++)
                for (int c = 0; c < 2; c++) {
                    value += pow(_u, a) * pow(_v, b) * pow(_w, c) * pow(1 - _u, 1 - a) * pow(1 - _v, 1 - b) *
                             pow(1 - _w, 1 - c) * Cell((i + a) % Size, (j + b) % Size, (k + c) % Size);
                }
...
private:
    float &Cell(unsigned int x, unsigned int y, unsigned int z) {
...
public:
    ...
    ProcMaterial(const Color &Diffuse, const Color &Specular, const Color &Ambient, float SpecularExp,
                 float OpticalDensityN)
    ...
    Color getDiffuseCoeff(const Vector &Pos) const {
        ...
        return m_DiffuseCoeff * sqrt(d) +
               m_DiffuseCoeff * 0.8f * (1 - sqrt(d));// Noisegrid.value(Pos.X , Pos.Y, Pos.Z);
...
Material::Material(const Color &Diffuse, const Color &Specular, const Color &Ambient, float SpecularExp,
                   float Reflectivity) {
...
Triangle::Triangle(const Vector &a, const Vector &b, const Vector &c, const Material *mtrl) : A(a), B(b), C(c),
                                                                                              pMtrl(mtrl) {
...
Scene::Scene(unsigned int SceneComplexity) {
    ...
    createInnerModel(InnerModelTriangles, 1.2f, SceneComplexity >= 3 ? SceneComplexity : 10,
                     SceneComplexity >= 3 ? SceneComplexity : 10);
    ...
    if (SceneComplexity >= 3)
        for (const auto &InnerModelTriangle: InnerModelTriangles)
            m_Triangles[i++] = InnerModelTriangle;
