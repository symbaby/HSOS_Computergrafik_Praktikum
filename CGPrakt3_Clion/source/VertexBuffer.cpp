...
void VertexBuffer::addNormal(float x, float y, float z) {
    if (!WithinBeginBlock) {
        std::cout << "call addNormal only between begin and end method!\n";
        return;
    }
...
void VertexBuffer::addColor(const Color &c) {
    if (!WithinBeginBlock) {
        std::cout << "call addColor only between begin and end method!\n";
        return;
    }
...
void VertexBuffer::addTexcoord0(float s, float t, float u) {
    if (!WithinBeginBlock) {
        std::cout << "call addTexcoord0 only between begin and end method!\n";
        return;
    }
...
void VertexBuffer::addTexcoord1(float s, float t, float u) {
    if (!WithinBeginBlock) {
        std::cout << "call addTexcoord1 only between begin and end method!\n";
        return;
    }
...
void VertexBuffer::addVertex(const Vector &v) {
    if (!WithinBeginBlock) {
        std::cout << "call addVertex only between begin and end method!\n";
        return;
    }
...
void VertexBuffer::end() {
    ...
    if (((ActiveAttributes & NORMAL) && (VertexCount != (unsigned int) Normals.size())) ||
        ...
        std::cout
                << "VertexBuffer::end(): vertex count and attribute(s) count do not match! Please ensure that #vertices=#normals, #vertices=#colors, #vertices=#texcoord\n";
        return;
    }
