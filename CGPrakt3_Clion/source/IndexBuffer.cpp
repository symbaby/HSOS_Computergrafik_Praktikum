...
IndexBuffer::IndexBuffer() : BufferInitialized(false), WithinBeginAndEnd(false), IndexFormat(GL_UNSIGNED_INT),
                             IndexCount(0) {
...
void IndexBuffer::addIndex(unsigned int Index) {
    if (!WithinBeginAndEnd) {
        std::cout << "call addIndex only between begin() and end() method!\n";
        return;
    }
